...
| `straight_line` | 8m 직선 코스, 노란 벽으로 통로 표시, 장애물 1개 | (-3.5, 0, 0°) | 직진 주행 + 장애물 감지 연습 (Phase 1~3) |
| `track` | 레이싱 트랙(차선/횡단보도/주차구획 텍스처), 장애물 없음 | (-0.51, -3.25, 0°) | 차선 주행 연습용 기본 트랙 |
| `track_obstacles` | `track`과 동일 + 장애물 2개(정답 경로 위) | (-0.51, -3.25, 0°) | 장애물 회피 연습 |
| `maze` | 10x10 절차적 생성 미로, 출구 있음 | (0, 0, 0°) | SLAM/미로 탈출 미션 (Phase 5) — 상세는 아래 참고 |
...
| `traffic_light` | `track` 바닥 + 신호등 1개(빨강 기본 점등) | (-0.51, -3.25, 0°) | 신호 인식/정지출발 연습, 색 변경법 아래 참고 |
